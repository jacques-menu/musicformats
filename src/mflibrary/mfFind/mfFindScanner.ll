...
%{
//   mfFind::parser::symbol_type JMI
//   make_NAME (const std::string &s, const mfFind::parser::location_type& loc);
%}
...
// A handy shortcut to the location held by the mfFindDriver
mfFind::location& loc = drv.getScannerLocationNonConst ();
...
<SINGLE_QUOTED_STRING_MODE>{singleleQuote} {
  ...
  return
    mfFind::parser::make_SINGLE_QUOTED_STRING (pStringBuffer,loc);
}
...
<DOUBLE_QUOTED_STRING_MODE>{doubleQuote} {
  ...
  BEGIN INITIAL;
  return
    mfFind::parser::make_DOUBLE_QUOTED_STRING (pStringBuffer, loc);
}
...
"(" {
  ...
  return
    mfFind::parser::make_LEFT_PARENTHESIS (loc);
}
...
")" {
  ...
  return
    mfFind::parser::make_RIGHT_PARENTHESIS (loc);
}

. {
   throw mfFind::parser::syntax_error (
     loc,
...
void mfFindDriver::scanEnd ()
{
  fclose (yyin);
}

//_______________________________________________________________________________
mfMusicformatsErrorKind launchmfFindInterpreter ()
{
  ...
  if (parseResult != 0) {
    ...
    result =
      theDriver.launchmfFindService_Pass2 ();
  }
