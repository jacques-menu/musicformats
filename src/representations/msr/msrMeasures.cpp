...
#ifdef MF_TRACE_IS_ENABLED
  if ( // JMI
    gTraceOahGroup->getTraceNoteDurations ()
      ||
  ...
  if (fMeasureIsFirstInVoice) {
    ...
    if (
      ...
      voice->
        incrementVoiceCurrentMeasurePuristNumber (
          inputLineNumber,
          "handleIncompleteMeasure 5 kMeasureEndRegularKindYes");
      ...
      case msrMeasureRepeatContextKind::kMeasureRepeatContext_UNKNOWN_: // JMI ???
        {
          ...
          if (gTraceOahGroup->getTraceMeasuresDetails ()) {
            displayMeasure (
              inputLineNumber,
              "handleIncompleteMeasure 6 kMeasureRepeatContext_UNKNOWN_");
          }
#endif // MF_TRACE_IS_ENABLED
...
/* JMI
      ...
      gLog <<
        "*** currentHarmony is null ***" << // JMI HARMFUL v0.9.67
        std::endl;
    }
    */
  ...
  const std::string&          context)
{
  ...
  msrWholeNotes
    measureWholeNotesFromPartMeasuresVector = // USELESS ??? JMI v0.9.66
      harmoniesPart->
        fetchPartMeasuresWholeNotessVectorAt (
          inputLineNumber,
          fMeasureOrdinalNumberInVoice - 1);
  ...
  // otherwise the last harmony in the measure has already been extended to the end of the measure
