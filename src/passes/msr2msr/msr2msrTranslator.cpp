...
//________________________________________________________________________
// void msr2msrTranslator::displayVoiceRepeatsStackSummary (
//   const mfInputLineNumber& inputLineNumber,
//   const std::string&       context)
// {
...
//
//   gLog <<
//     ">>++++++++++++++++ displayVoiceRepeatsStackSummary() - Repeats stack of voice " <<
//     getVoiceName () <<
//     ", " <<
//     mfSingularOrPlural (voiceRepeatsStackSize, "element", "elements") <<
//     " - " << context <<
//     ", " << inputLineNumber <<
//     ':' <<
//     std::endl;
...
//       ++gIndenter;
//       gLog <<
//         repeat->asString () << // JMI 0.9.76 causes havoc???
//         std::endl;
//       --gIndenter;
//
//       --n;
...
//________________________________________________________________________
void msr2msrTranslator::displayMultiMeasureRestClones (
  const mfInputLineNumber& inputLineNumber,
  const std::string&       context)
{
  gLog <<
  ...
  const S_msrSegment&       segment)
{
  ...
  fSegmentsStack.push_front (segment);
}
...
void msr2msrTranslator::displaySegmentsStack (
  ...
  const std::string&       context)
{
  size_t voicePendingSegmentsStackSize =
    fSegmentsStack.size ();

  gLog <<
    ">>++++++++++++++++ displaySegmentsStack() - Segments stack of voice " <<
    fCurrentVoiceClone->getVoiceName () <<
    ", " <<
    mfSingularOrPlural (voicePendingSegmentsStackSize, "element", "elements") <<
    " - " << context <<
    ", " << inputLineNumber <<
    ':' <<
    std::endl;
  ...
  if (voicePendingSegmentsStackSize) {
    std::list <S_msrSegment>::const_iterator
      iBegin = fSegmentsStack.begin (),
      iEnd   = fSegmentsStack.end (),
      i      = iBegin;

    int n = voicePendingSegmentsStackSize;
    for ( ; ; ) {
      S_msrSegment segment = (*i);
...
//________________________________________________________________________
void msr2msrTranslator::pushMeasureOntoMeasuresStack (
  const mfInputLineNumber&  inputLineNumber,
  const S_msrMeasure&       mesure)
{
  ...
  if (gTraceOahGroup->getTraceMeasuresBasics ()) {
    ...
    ss <<
      "Pushing mesure " <<
      mesure->asShortString () <<
      " onto mesure clones stack" <<
      ", " << inputLineNumber;
  ...
  fMeasuresStack.push_front (mesure);
}

S_msrMeasure msr2msrTranslator::popMeasureFromMesuresStack (
  const mfInputLineNumber& inputLineNumber)
{
#ifdef MF_SANITY_CHECKS_ARE_ENABLED
  // sanity check
  mfAssert (
    __FILE__, mfInputLineNumber (__LINE__),
    ! fMeasuresStack.empty (),
    "fMeasuresStack is EMPTY");
#endif // MF_SANITY_CHECKS_ARE_ENABLED
...
void msr2msrTranslator::displayMeasuresStack (
  ...
  const std::string&       context)
{
  size_t voicePendingMeasuresStackSize =
    fMeasuresStack.size ();

  gLog <<
    ">>++++++++++++++++ displayMeasuresStack() - Measures stack of voice " <<
    fCurrentVoiceClone->getVoiceName () <<
    ", " <<
    mfSingularOrPlural (voicePendingMeasuresStackSize, "element", "elements") <<
    " - " << context <<
    ", " << inputLineNumber <<
    ':' <<
    std::endl;
  ...
  if (voicePendingMeasuresStackSize) {
    std::list <S_msrMeasure>::const_iterator
      iBegin = fMeasuresStack.begin (),
      iEnd   = fMeasuresStack.end (),
      i      = iBegin;

    int n = voicePendingMeasuresStackSize;
    for ( ; ; ) {
      S_msrMeasure mesure = (*i);

      gLog <<
        "v (" << n << ")" <<
        ", getInputLineNumber: " <<
        mesure->getInputLineNumber () <<
        std::endl;
...
//________________________________________________________________________
void msr2msrTranslator::pushRepeatOntoRepeatsStack (
  const mfInputLineNumber& inputLineNumber,
  ...
  const std::string&       context)
{
  ...
  fRepeatsStack.push_front (repeat);

#ifdef MF_TRACE_IS_ENABLED
  if (gTraceOahGroup->getTraceRepeatsBasics ()) {
    std::string
      combinedContext =
        "pushRepeatOntoRepeatsStack() END, context: " + context;
  ...
  const std::string&       context)
{
  ...
  if (gTraceOahGroup->getTraceRepeatsBasics ()) {
    std::string
      combinedContext =
        "popRepeatFromRepeatsStack() BEGIN, context: context " + context;
  ...
  if (gTraceOahGroup->getTraceRepeatsBasics ()) {
    std::string
      combinedContext =
        "popRepeatFromRepeatsStack() END, context: " + context;
  ...
  const std::string&       context)
{
  ...
  gLog <<
    ...
    " (context: " << context << ")" <<
    ", " <<
    mfSingularOrPlural (voicePendingRepeatsStackSize, "element", "elements") <<
    " - " << context <<
    ", " << inputLineNumber <<
    ':' <<
    std::endl;
  ...
  if (voicePendingRepeatsStackSize) {
    ...
    int n = voicePendingRepeatsStackSize;
    for ( ; ; ) {
      S_msrRepeat repeat = (*i);
  ...
  const S_msrRepeatElement& repeatElement)
{
  ...
  if (gTraceOahGroup->getTraceRepeatsBasics ()) {
    ...
    ss <<
      "Pushing repeat element " <<
      repeatElement->asShortString () <<
      " onto repeat elements stack" <<
      ", " << inputLineNumber;
  ...
  fRepeatElementsStack.push_front (repeatElement);
}
...
//________________________________________________________________________
void msr2msrTranslator::handleNestedRepeatStartInVoice (
  const mfInputLineNumber& inputLineNumber)
{
#ifdef MF_TRACE_IS_ENABLED
  if (gTraceOahGroup->getTraceRepeatsBasics ()) {
  ...
  const std::string&       context)
{
  ...
  if (fCurrentMeasureRepeatClone) {
    fCurrentMeasureRepeatClone->
...
void msr2msrTranslator::visitStart (S_msrSegment& elt)
{
  ...
  pushSegmentOntoSegmentsStack (
    elt->getInputLineNumber (),
    segmentClone);

  //  place it in the voice clone or in the inner-most repeat element
  switch (elt->getSegmentKind ()) {
...
void msr2msrTranslator::visitEnd (S_msrMeasure& elt)
{
  ...
  if (gTraceOahGroup->getTraceMeasuresBasics ()) {
    ...
    ss <<
      ...
      ", " << currentMeasureClone->getInputLineNumber ();
...
void msr2msrTranslator::visitEnd (S_msrGraceNotesGroup& elt)
{
  ...
  if (gTraceOahGroup->getTraceGraceNotes ()) {
    ...
    ss <<
      ...
      std::endl <<
      "fCurrentNonGraceNoteClone: " <<
      fetchNoteAsString (fCurrentNonGraceNoteClone);
        ...
        if (fCurrentNonGraceNoteClone->getNoteIsFirstNoteInADoubleTremolo ()) {
          ...
          if (gTraceOahGroup->getTraceNotes ()) {
            ...
            ss <<
              "Setting note " <<
              fCurrentNonGraceNoteClone->asString () <<
              ", " << fCurrentNonGraceNoteClone->getInputLineNumber () <<
...
void msr2msrTranslator::visitStart (S_msrTuplet& elt)
{
  ...
  fTupletsStack.push_front (tupletClone);
}
...
void msr2msrTranslator::visitStart (S_msrRepeatEnding& elt)
{
  ...
  S_msrRepeatEnding
    repeatEndingClone =
      elt->createRepeatEndingNewbornClone (
        fCurrentVoiceClone);
...
//         fCurrentSegmentClone);
  ...
  fRepeatsStack.front ()->
    appendMultiMeasureRestToRepeat (
      elt->getInputLineNumber (),
      fCurrentMultiMeasureRestsClone,
      "visitStart (S_msrMultiMeasureRest& elt)");
...
//
//         // create a multi-measure rests  containing fMeasuresStack.front ()
...
//       }
//
//       fCurrentMultiMeasureRestsClone = fMeasuresStack.front ();
//     }
//
//     else {
//       // NO
