...
void msr2msrOahGroup::initializeTemposOptions ()
{
  S_oahSubGroup
    subGroup =
      oahSubGroup::create (
        "Tempos",
        "help-msr2msr-tempos", "hmsrtempos",
R"()",
      oahElementVisibilityKind::kElementVisibilityWhole,
      this);
  ...
  subGroup->
    appendAtomToSubGroup (
      oahBooleanAtom::create (
        "convert-tempos-to-rehearsal-marks-in-msr-in-msr", "cttrmim",
R"(Convert tempos to rehearsal marks in the MSR.
...
R"()",
  ...
  subGroup->
    appendAtomToSubGroup (
      oahBooleanAtom::create (
        "convert-words-to-tempo-in-msr", "cwttim",
R"(Convert words to tempo in the MSR.
This may come in handy when MusicXML data has been obtained
from scanned instrumental music images.)",
        "fConvertMsrWordsToTempo",
        fConvertMsrWordsToTempo));
...
This may come in handy when MusicXML data has been obtained from scanned images.)",
  ...
  subGroup->
    appendAtomToSubGroup (
      oahBooleanAtom::create (
        "convert-msr-words-to-rehearsal-marks", "cmwtrm",
R"(Convert MSR words to rehearsal marks.
This may come in handy when MusicXML data has been obtained
from scanned instrumental music images.)",
        "fConvertMsrWordsToRehearsalMarks",
        fConvertMsrWordsToRehearsalMarks));
