...
\tikzmath{
  ...
  % Mxsr
  \MxsrAngle = 90;
  \MxsrAbs = cos(\MxsrAngle);
  \MxsrOrd = sin(\MxsrAngle);
  %
  % MusicXML
  \MusicXMLAbs = \MxsrAbs * 2;
  \MusicXMLOrd = \MxsrOrd * 2;
  %
  % Guido
  \GuidoAngle = 55;
...
\node[align=center,style={circle,minimum size=80,fill=red!15}]
  (Mxsr)
  at (\MxsrAbs,\MxsrOrd) {xmlelement\\tree};
...
% MusicXML <-> Mxsr
\draw [<->,thick,{Stealth[length=10pt]}-{Stealth[length=10pt]}] (MusicXML) -- (Mxsr);

% Mxsr -> MSR
\draw [->,thick,{Stealth[length=10pt,color=gray!50]}-{Stealth[length=10pt]}] (Mxsr) -- (MSR);
...
\hline\\[-3.0ex]
%
MXSR & MusicXML Score Representation, a tree representing the MusicXML markups such as {\tt <part-list>}, {\tt <time>} and {\tt <note>}
\tabularnewline
...
\texttt{RandomMusic} & generates an MXSR containing random music and writes it as MusicXML
\tabularnewline
