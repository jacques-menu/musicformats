...
Note that two \musicXmlMarkup{direction-type} elements are needed, since only one of \musicXmlMarkup{words} and \musicXmlMarkup{metronome} can be present in a given instance, as stated in \schemafile{direction.mod}:
\begin{lstlisting}[language=MusicXML]
...
\item \textcolor{blue}{blue}: a link from a class to another that uses smart pointers to one or more instances the former. For example, an \class{msrTuplet} may be an element of an \class{msrGraceNotesGroup}.
\end{itemize}
...
Part groups handling is presented at \sectionRef{Part groups}.
...
Full measure rests handling is presented at \sectionRef{Full measure rests handling}.

\Class{msrFullMeasureRests}, defined in \msrBoth{msrFullMeasureRests}, essentially contains a \class{msrFullMeasureRestsContents} instance and a mesure rests number:
\begin{lstlisting}[language=CPlusPlus]
...
For example, at \figureRef{Three segments in a voice}, there are three segments:\begin{itemize}
