...
\item classes constant fields such as \constant{msrStanza}{K_STANZA_NUMBER_UNKNOWN}, to be prefered to C-style {\tt \#define} preprocessor specifications for type safety;
...
Enumeration constants in the form {\tt k*_UNKNOWN} are used to indicate a value that \MainIt{has not been set yet}. There are always the first on in the corresponding enumeration, to benefit from the the \CPlusplus\ implicit \initialization\  to the the equivalent of 0.
...
class   {\tt mfRational} is defined by \libmusicxml\ in \libBoth{mfRational}:
\begin{lstlisting}[language=CPlusPlus]
