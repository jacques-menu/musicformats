...
\Function{catchSignals} at the beginning of the \functionName {main} function can be commented out in order to run a tool under a debugger.

File \wae{waeEnableAbortToDebugErrors.h} contains:
...
(lldb) settings set -- target.run-args  "fullbarrests/FullBarRestWithoutBarLine.xml" "-display-lpsr-short" "-aofn"
(lldb) r
...
* thread #1, queue = 'com.apple.main-thread', stop reason = EXC_BAD_ACCESS (code=1, address=0x1)
    frame #0: 0x00000001007b879a xml2ly`MusicFormats::msrPartGroup::printShort(this=0x0000600003e08100, os=0x000000010ca04650) const at msrPartGroups.cpp:1185:13
   1182	      i      = iBegin;
   1183
   1184	    for ( ; ; ) {
-> 1185	      (*i)->printShort (os);
...
The bug shows up with \mfFiles{musicxmlfiles/fullbarrests/FullBarRestWithoutBarLine.xml}, but not with others such as \mfFiles{musicxmlfiles/multistaff/SATBExample.xml}.
