...
The \component s of \mf\ are described by \enumType{mfcComponenKind}:
\begin{lstlisting}[language=CPlusPlus]
...
The \purelyVirtualClass{mfcComponentDescr} is a superclass to the ones describing formats, passes, generators, converters and the \mf\ library itself:
\begin{lstlisting}[language=CPlusPlus]
class mfcComponentDescr : public smartable
{
	// ... ... ...
...
Multi-components have their own history, hence field \method{mfcComponentDescr}{printOwnHistory}.
...
\begin{lstlisting}[language=CPlusPlus]
class mfcMultiComponent : public mfcComponentDescr
{
	// ... ... ...
...
\Field{oahHandler}{fHandlerMultiComponent} is set in the \className{oahHandler} subclasses constructors by a call to the overriden \methodName{initializeHandlerMultiComponent}.
