...
The distributions \zip\ archives are supplied with all \mf\ versions, i.e. the current, most recent version of \mf\ (the \default\ \masterBranch\ in the \repo), and the earlier versions such as the \code{v0.9.60} branch.
...
For example, the \Ubuntu\ action in \file{ubuntu-check.yml} is shown below. It is executed each time a \code{git push} is performed to the \masterBranch:
\begin{lstlisting}[language=Terminal]
...
\begin{lstlisting}[language=Terminal]
jacquesmenu@macmini: ~/musicformats-git-dev/distrib > ../build/scripts/MakeMusicFormatsDistributions.bash
... ... ...
