...
\mfslLang\ is implemented with the \flex\ and \bison\ code generators:
\begin{itemize}
\item \mfsl{mfslScanner.l} contains the \flex\ lexical description of \mfslLang. It is used to create \mfsl{mfslScanner.cpp};

\item \mfsl{mfslParser.y} is the syntax and semantics description of \mfslLang. From it, \bison\ creates \mfsl{mfslParser.h}, \mfsl{mfslParser.cpp} and \mfsl{mfslParser.output}. 
...
This \Makefile\ is quite simple: the options to \flex\ and \bison\ are placed in \mfsl{mfslScanner.l} and \mfsl{mfslParser.y}, respectively:
...
\item \mfsl{mfslDriver.cpp} contains the \className{mfslDriver} \constructor, that runs the parser:
\begin{lstlisting}[language=Terminal]
...
\item the remaining code is placed in the third part of (service code) of \mfsl{mfslScanner.l}, since it needs to access variables in the code generated by \flex:
\begin{lstlisting}[language=Terminal]
...
The definition of this function is placed in \mfsl{mfslScanner.l}:
\begin{lstlisting}[language=Terminal]
