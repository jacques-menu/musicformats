...
% -------------------------------------------------------------------------
\chapter{Maintenance scripts}\label{Maintenance scripts}
% -------------------------------------------------------------------------

They are located in \scriptsFolder\ and meant to facilitate the \mf\ maintenance activities:
\begin{lstlisting}[language=Terminal]
jacquesmenu@macstudio-1:/Volumes/JMI_Volume/JMI_Developpement/musicformats-git-dev/scripts > ls -sal
total 120
...
\scripts{BuildMusicFormats.zsh} executes \cmake, whose configuration is in \buildFolder:
\begin{lstlisting}[language=Terminal]
...
\scripts{CreateDocumentationPDFs.zsh} executes lives up to its name. The \LaTeX\ source files are in \docFolder:
\begin{lstlisting}[language=Terminal]
...
When modifying a documentation's structure information, i.e. chapter, section and \code{\subsection} commands contents, \code{pdflatex} should be run three times to get the corresponding PDF.
...
\scripts{CreateMusicFormatsReleases.zsh} lives up to its name too. 
...
\scripts{LocateFilesContainingFirstAndNotSecond.zsh} helps to find superfluous \CPlusplus\ includes or other such checking tasks. 
...
There are a couple of them in \scripts{ZshDefinitionsForMusicFormats.zsh}
