...
../src/elements/templates/elements.bash elements.txt ../src/elements/templates std::map > factory.cpp || rm -f factory.cpp
\end{lstlisting}

The resulting files are the following, where \fileName{elements.txt} contains an alphabetic std::list of the \mxml\ markups found in the DTD:
\begin{lstlisting}[language=Terminal]
jacquesmenu@mac-mini-de-jacques-1:~/musicformats-git-dev/libmusicxml/schema > ls -sal
total 208
...
In that case, the corresponding constants can be found in \fileName{elements.h}, at line '26+numericalValue'. For example, with  \mxml\ version 4.0, 241 is the numerical value of \code{k_notations}, describing markup \musicXmlMarkup{"notations"}:
\begin{lstlisting}[language=TerminalSmall]
...
In the \fileName{scripts} folder, \fileName{elements.bash} compiles the definitions of MusicXML markups into the C++ code files containing the corresponding C++ classes:
\begin{lstlisting}[language=Terminal]
