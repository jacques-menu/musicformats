...
\item a \class{oahHandler} contains a std::list of {\tt oahGroup}'s, each handled
    in a std::pair of {\tt .h/.cpp} files such as \msr{msrOah.h} and \msr{msrOah.cpp},
    and a std::list of options prefixes.

\item a \class{oahGroup} contains a std::list of {\tt oahSubGroup}'s
    and an upLink to the containing {\tt oahHandler}.

\item a \class{oahSubGroup} contains a std::list of {\tt oahAtom}'s
...
\item a \class{oahCombinedBooleansAtom} contains a std::list of boolean atoms
    to manipulate several such atoms as a single one,
    see the '\class{cubase}' combined booleans atom in \mxsrToMsr{mxsr2msrOah.cpp};

\item \class{oahMultiplexBooleansAtom} contains a std::list of boolean atoms
...
Some options expect a value, such a length or a color, to be supplied in the \CLI\ or in a \type{oahOptionsVector}.

\PurelyVirtualClass{oahAtomExpectingAValue}, defined in \oahBoth{oahBasicTypes}, is a common ancestor to all the classes describing such options:
...
The classes derived from \className{oahAtomExpectingAValue} are:
...
The \field{oahAtomStoringAValue}{fSetByAnOption} is necessary because some value types do not have an obvious 'neutral' element. This is the case for a note's octave, \className{oahLengthUnitKindAtom} and \className{oahColorRGBAtom}, for example. It is not used for data structures such as sets, lists and std::vector, since this is indicated by their size.

\fieldName{fSetByAnOption} is set in \methodName{set*Variable} methods, as in \method{oahIntegerAtom}{setIntegerVariable}\\
...
\Class{oahBooleanAtom} has its own \fieldName{fSetByAnOption} field, because is it not derived from\\
\class{oahAtomStoringAValue}: there isn't any value to be supplied in the command line, since \fieldName{fBooleanVariable} is \code{false} by default:
\begin{lstlisting}[language=CPlusPlus]
...
This in done by calls to the \methodName{getSetByAnOption} methods.
...
The default \lily\ version number is \code{2.22.0}. Another can be chosen with the \optionNameBoth{lilypond-version}{lpv} option:
\begin{lstlisting}[language=Terminal]
...
The base \className{oahPureHelpAtomWithoutAValue} class   is defined in \oahBoth{oahBasicTypes}:
\begin{lstlisting}[language=CPlusPlus]
//______________________________________________________________________________
...
The base \className{oahPureHelpAtomExpectingAValue} class   is defined in \oahBoth{oahBasicTypes}:
\begin{lstlisting}[language=CPlusPlus]
...
\PurelyVirtualClass{oahAtomImplicitlyStoringAValue} is the base class   for this:
\begin{lstlisting}[language=CPlusPlus]
...
\item options handling works in two passes:
    \item the first one creates a std::list of \class{oahElementUse} instances from
...
\item \method{oahHandler}{handleKnownArgvAtom} associatiates the value
    to the (preceding) \field{oahHandler}{fPendingArgvAtomExpectingAValue} if not null,
...
\item in the generators CLI tools found in the \clisamples{} folder \clisamples{xml2Any.cpp}, \clisamples{libMultipleInitsTest.cpp}, \clisamples{Mikrokosmos3Wandering.cpp} and \clisamples{LilyPondIssue34.cpp}, as well as in \clisamples{msdl.cpp}, the MSDL converter command line interface.
\end{itemize}

 In these tools, and {\tt oahOptionsVector} is instantiated ans populated from \argcargv\ with \functionName{convertArgcArgvToOptionsAndArguments}, defined in \oah{oahBasicTypes.h}:%%%JMI
\begin{lstlisting}[language=CPlusPlus]
...
Atoms that can have an associated value are described in \oahBoth{oahBasicTypes} by class   {\tt oahAtomExpectingAValue}, which has methods {\tt applyAtomWithValue} and {\tt applyAtomWithDefaultValue}:
\begin{lstlisting}[language=CPlusPlus]
    virtual void          applyAtomWithValue (
                            const std::string& theString,
                            std::ostream& os) = 0;
...
The last option is checked by \method{oahHandler}{checkMissingPendingArgvAtomExpectingAValueValue} in \oah{oahBasicTypes.cpp}. %%% JMI
...
For example, in \xmlToXmlTool{xml2xmlRegularHandler.cpp}, there is:
\begin{lstlisting}[language=CPlusPlus]
...
Debugging OAH needs the trace handling tracing options \optionBoth{trace-oah}{toah} and \optionBoth{trace-oah-details}{toahd} to be activated first, even if they are not the first ones supplied.
...
When matches are found, there are stored in a std::list of instances of \className{oahFindStringMatch}:
\begin{lstlisting}[language=Terminal]
...
The \fieldName{fContainingFindableElementInfo} describes the \oahRepr\ element that the std::string was found in, either in its option name(s) or in its description.

For example, method \method{oahHandler}{findStringInFindableElement} in \oah{oahElement.cpp} creates an instance and appends it to the std::list :
\begin{lstlisting}[language=Terminal]
...
For example, the tracing of \musicXmlMarkup{backup} and \musicXmlMarkup{forward} is used by both \fileName{mxsr2msrOah} and \fileName{msr2mxsrOah}. The corresponding options are thus placed in \mxsrBoth{mxsrOah}:
\begin{lstlisting}[language=CPlusPlus]
...
\item it was decided to place it in the \globalVariable{gGlobalOahOahGroup} \oahRepr\ group, in its {\tt Options~help} sub group;
...
% -------------------------------------------------------------------------
\subsection{Creating a new OAH atom class EXPecting a value}
% -------------------------------------------------------------------------
...
\Method{oahLengthAtom}{applyAtomWithValue} in \oah{oahAtomsCollection.cpp} deciphers the std::string supplied by the user and stores it the \class{msrLength} variable:
\begin{lstlisting}[language=CPlusPlus]
...
\Method{oahLengthAtom}{printAtomWithVariableOptionsValues} is in charge of displaying the length value when \option{display-options-values}{dov} is chosen:
\begin{lstlisting}[language=CPlusPlus]
...
Macro {\tt EXTRA_OAH_IS_ENABLED} is defined or not in \oah{enableHarmoniesExtraOahIfDesired.h}:
\begin{lstlisting}[language=CPlusPlus]
...
\mf\ can create man pages for its \CLI\ tools by browing their \oahRepr\ hierarchy. This has not been finalized yet.%%%JMI
