...
\begin{itemize}
\item \colorbox{black}{\textcolor{green}{green}}: a OAH element that is expected to be found in a options and help user view, such as \class{msrStaff} and \class{msrChord};

\item \colorbox{black}{\textcolor{pink}{pink}}: a element needed in OAH to structure the OAH data structures, such as \class{msrSegment} and \class{msrSyllable};
...
\begin{itemize}
\item \textcolor{red}{red}: a link from a class to its base class. For example, \class{msrPart} is derived from \class{msrPartGroupElement};

\item \textcolor{blue}{blue}: a link from a class to another that uses smart pointers to one or more instances of the former. For example, an \class{msrTuplet} may be an element of an \class{msrGraceNotesGroup}.
\end{itemize}

Here is the essentials of the \oahRepr\ classes:
