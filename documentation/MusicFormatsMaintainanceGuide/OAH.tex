...
\item there are various subclasses of \class{oahAtom} such as \class{oahIntegerAtom}, \class{oahBooleanAtom}
...
The \field{oahAtomStoringAValue}{fVariableHasBeenSet} is used because some value types do not have an obvious 'neutral' element. This is the case for a note's octave, \className{oahLengthUnitKindAtom} and \className{oahRGBColorAtom}, for example.

It is set in \methodName{set*Variable} methods, as in \method{oahIntegerAtom}{setIntegerVariable}\\
...
\Class{oahBooleanAtom} has its own \fieldName{fVariableHasBeenSet} field, because is it not derived from\\
...
This in done by calls to the \methodName{getVariableHasBeenSet} methods.
...
oah/oahAtomsCollection.h:class EXP oahNaturalNumbersSetAtom : public oahAtomStoringAValue
oah/oahAtomsCollection.h:class EXP oahRGBColorAtom : public oahAtomStoringAValue
oah/oahAtomsCollection.h:class EXP oahIntSetAtom : public oahAtomStoringAValue
oah/oahAtomsCollection.h:class EXP oahStringSetAtom : public oahAtomStoringAValue
oah/oahAtomsCollection.h:class EXP oahStringToIntMapAtom : public oahAtomStoringAValue
...
passes/mxsr2msr/mxsr2msrOah.h:class EXP msrDalSegnoAtom : public oahAtomStoringAValue
...
\item application using the library through the \API\ should place them in an {\tt oahOptionsVector}, defined in \mflibrary{mfMusicformatsError.h}:
\begin{lstlisting}[language=CPlusPlus]
...
Handling these options ahead of regular options handling is easily done in the \mainFunction\ functions\index{functions} of the CLI tools, as in \clisamples{xml2ly}:%%%JMI
\begin{lstlisting}[language=CPlusPlus]
  ...
  Bool insiderOption;
    ...
    if (
      argumentWithoutDash == K_TRACE_OAH_DETAILS_SHORT_OPTION_NAME
        ||
      argumentWithoutDash == K_TRACE_OAH_DETAILS_LONG_OPTION_NAME
    ) {
      setEarlyTracingOahDetails ();
    }

    if (
      argumentWithoutDash == K_TRACE_COMPONENTS_SHORT_OPTION_NAME
        ||
      argumentWithoutDash == K_TRACE_COMPONENTS_LONG_OPTION_NAME
    ) {
      setEarlyTraceComponents ();
    }
#endif

		if (argumentWithoutDash == K_INSIDER_OPTION_LONG_NAME) {
		  insiderOption = true;
		}
%		if (argumentWithoutDash == K_REGULAR_OPTION_LONG_NAME) {
%		  regularOption = true;
%		}
	} // for
\end{lstlisting}
