...
\item optionaly, any number of \code{select} or an \code{every} statement.
...
\begin{lstlisting}[language=MFSL]
jacquesmenu@macmini: ~/musicformats-git-dev/files/mfslfiles > cat Hymn121ForBass.mfsl
#!//Users/jacquesmenu/musicformats-git-dev/build/bin/mfsl
...
\begin{lstlisting}[language=Terminal]
jacquesmenu@macmini: ~/musicformats-git-dev/files/mfslfiles > ./Hymn121ForBass.mfsl
...
jacquesmenu@macmini: ~/musicformats-git-dev/files/mfslfiles > ls -sal Hymn121_Bass.ly
...
\begin{lstlisting}[language=Terminal]
jacquesmenu@macmini: ~/musicformats-git-dev/files/mfslfiles > diff Hymn121ForBass.mfsl Hymn121ForBassEnhanced.mfsl 
35c35
<   -output-file-name "Hymn121_Bass.ly"
---
...
\begin{itemize}
\item there can be either a group of several \code{select} statements or one \code{every} statement at the end of the \script;
...
\item if none of \code{select} nor \code{every} is present, the default \coiceLabel\ for the \choice\ is implicitly used.
\end{itemize}
...
With:
\begin{lstlisting}[language=Terminal]
./Hymn121ForVocalSATBQuartet.mfsl -every SCORE
\end{lstlisting}

we get all these \lily\ files:
\begin{lstlisting}[language=TerminalSmall]
jacquesmenu@macmini: ~/musicformats-git-dev/files/mfslfiles > ls -sal Hymn121_SATB_*.ly
...
\begin{lstlisting}[language=Terminal]
jacquesmenu@macmini: ~/musicformats-git-dev/files/mfslfiles > cat minimal.bash 
#!/bin/bash
...
\begin{lstlisting}[language=Terminal]
jacquesmenu@macmini: ~/musicformats-git-dev/files/mfslfiles > ./minimal.bash 
+ MINIMAL_LY=Minimal.ly
+ ./minimal.mfsl
+ ls -sal Minimal.ly
...
\begin{lstlisting}[language=TerminalSmall]
jacquesmenu@macmini: ~/musicformats-git-dev/files/mfslfiles > ls -l Hymn121_harmony_*.ly
...
\item the second pass uses the informations thus collected to launch the tool as many times as needed, depending on \code{select} and \code{every} usage in the script body and the options to the \mfslInterp.
\end{enumerate}
