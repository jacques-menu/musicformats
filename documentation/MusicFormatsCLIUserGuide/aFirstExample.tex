...
In computer science, the simplest example one can write with a language is often named 'Hello World'. \mf\ abides to this rule, suppling \mxmlfile{basic/HelloWorld.xml}:
...
By default, the standard output and error streams are directed to the terminal in which the \xmlToLy\ command has been submitted.

Let's consider the case of \mxmlfile{basic/UnknownMaintainerIdentification.xml}:
\begin{lstlisting}[language=Lilypond]
jacquesmenu@macmini: ~/musicformats-git-dev/files/musicxmlfiles > xml2ly basic/UnknownMaintainerIdentification.xml
...
The standard output and error streams are merged into the terminal window, which may not be satisfactory.
...
\item \code{2>}: redirects the standard error stream to a file.
\end{itemize}
...
options:
\begin{lstlisting}[language=Terminal]
jacquesmenu@macmini-1: ~/musicformats-git-dev/files/musicxmlfiles > xml2ly -auto-output-file-name -trace-passes -display-cpu-usage basic/Anacrusis.xml
...
% -------------------------------------------------------------------------
\section{More converters and options}
% -------------------------------------------------------------------------
...
\begin{lstlisting}[language=Terminal]
jacquesmenu@macmini: ~/musicformats-git-dev/files/musicxmlfiles > xml2ly basic/MinimalScore.xml -output-file-name MinimalScore_Jianpu.ly -jianpu -title "Minimal score in Jianpu format"
\end{lstlisting}

The key in this example is E major. The resulting \fileName{MinimalScore_Jianpu.ly} leads to:\\
\includegraphics[scale=1]{../graphics/MinimalScore_Jianpu.png}

to be compared with:\\
\includegraphics[scale=1]{../graphics/MinimalScore.png}

%The same score can also be produced in braille by \xmlToBrl:
%\begin{lstlisting}[language=Terminal]
%jacquesmenu@macmini: ~/musicformats-git-dev/files/musicxmlfiles > xml2brl basic/MinimalScore.xml -auto-output-file-name -utf8d --use-encoding-in-file-name
%\end{lstlisting}
