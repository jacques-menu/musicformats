...
\item the options can be supplied in the \CLI\ as usual;
  ...
  \item they can also be supplied in a call to an \API\ \Main{function} such as \functionName{musicxmlfile2lilypond}, in an options and arguments argument. \\
    		See the \gitdocpdf{MusicFormatsAPIUserGuide}{MusicFormatsAPIUserGuide} for the details;
...
The score is to be used by a musician who needs very readable scores with large note heads and alterations, hence the \optionName{global-staff-size}, \optionName{page-count}, \optionName{ragged-last-bottom} and \optionName{repeat-brackets} options.

And finally, the slurs in the original were wrong, i.e. not the {\it \swing\ \phrase} as should be, hence the use of \option{ignore-slurs}:
\begin{lstlisting}[language=Terminal]
