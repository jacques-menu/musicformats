...
\code{cmake} is used to implement the portability of \mf\ to multiple \OS s and environments. \code{make} is used to build the library. Thanks to \fober\ for providing this in \libmusicxml\ in the first place.
The respective settings are in \build{CMakeLists.txt} and \build{Makefile}.

The \code{make} file as a number of possibilities>
\begin{lstlisting}[language=Terminal]
...
\item the \fileName{cmake} tool. It is available ready to install on \MacOS\ via \macports\ (\url{https://www.macports.org}).
\end{itemize}
...
Then install \mf\ either globally on you machine or in the user's home directory:
\begin{lstlisting}[language=Terminal]
jacquesmenu@macmini: ~/musicformats_local_clone/build > make install
\end{lstlisting}
or:
\begin{lstlisting}[language=Terminal]
