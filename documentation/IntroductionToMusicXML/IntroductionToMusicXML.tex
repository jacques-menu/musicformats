...
All the examples mentioned can be downloaded from \url{https://github.com/jacques-menu/musicformats/tree/master/files/musicxml}. They are grouped by subject in subdirectories, such as \mxmlfile{basic/HelloWorld.xml}.
...
The \subdir{dtds/3.1/schema} subdirectory contains '\fileName{\tt *.mod}' text files defining the various concepts. The file \schemafile{common.mod} contains definitions used in other '\fileName{\tt *.mod}' files:
\begin{lstlisting}[language=MusicXML]
<!--
...
For example, \schemafile{note.mod} defines the \musicXmlMarkup{backup} and \musicXmlMarkup{forward} markups this way:
\begin{lstlisting}[language=MusicXML, caption={\musicXmlMarkup{backup} and \musicXmlMarkup{forward} definition}]
<!--
...
\begin{lstlisting}[language=MusicXML, caption={\musicXmlMarkup{backup} and \musicXmlMarkup{forward} example}]
      <forward>
        <duration>4</duration>
        <voice>2</voice>
        <staff>1</staff>
...
For example, the first note of \mxmlfile{lyrics/QuemQueritis.xml} contains the single word '{\tt Quem}':\\
...
\mxml\ supports other score elements such as harmonies and figured bass, as well as nested repeats. There is a lot of information about \mxml\ on the Internet. And of course, plenty of targeted, ready-to-use examples can be found in \subdir{files/musicxml}.
