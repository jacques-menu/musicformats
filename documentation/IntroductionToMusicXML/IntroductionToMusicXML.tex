...
The '{\tt  <identification>}' element is defined in \gitschemafile{partGroupSequentialNumber.mod}:
\begin{lstlisting}[language=MusicXML]
<!--
...
There are no structured staves nor voices as such in \mxml\ however -- that is, not the way parts and measures are. The '{\tt <staff>}' and '{\tt voice}' element only contain a number.

\sep

To be more precise:
\begin{itemize}
\item staff numbers start at '{\tt 1}' in every part, which refers to the top-most staff in the part;

\item a staff number of '{\tt 1}' is implied by default, i.e. when an optional '{\tt <staff>}' element is missing, as can happen in notes descriptions;
...
Multiple measure rests are specified in \mxml\ with the '{\tt <multiple-rest>}' element. All measures in the sequence have to be explicitly present in the \mxml\ data.

\sep

For example, the first two measures of \gitmxmlfile{rests/MultipleMeasureRests.xml} are a multiple measure rest, described by:
\begin{lstlisting}[language=MusicXML]
  <part id="P1">
    <measure number="1">
      <attributes>
...
\includegraphics[scale=0.55]{MultipleMeasureRestsMuseScore.png}

\sep \sep

\mxmlToLy\ produces:\\
\includegraphics{MultipleMeasureRestsMusicxml2ly.png}

\sep \sep

\sib\ produces:\\
\includegraphics[scale=0.525]{MultipleMeasureRestsSibelius.png}

\sep \sep

\fin\ produces:\\
\includegraphics[scale=0.8]{MultipleMeasureRestsFinale.png}

\sep \sep

\xmlToLy\ is still experimental, and currently produces:\\
\includegraphics{MultipleMeasureRestsXml2ly.png}
