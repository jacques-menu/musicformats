...
The start-tag is introduced by a '{\tt <}' and closed by a '{\tt >}', as in '\musicXmlMarkup{<part-std::list>}'. The stop-tag is introduced by a '{\tt </}' and closed by a '{\tt >}', as in '\musicXmlMarkup{</part-std::list>}'.
...
\begin{itemize}
  ...
  \item a '{\tt <part-std::list>}' element containing the various '{\tt <score-part>}'s in the score;
  \end{itemize}
...
<!--
	...
	and the part std::list.
-->
<!ENTITY % score-header
	"(work?, movement-number?, movement-title?,
	  identification?, defaults?, credit*, part-std::list)">
\end{lstlisting}
...
Then the '{\tt <part-std::list>}' element at lines 7 to 11 contains a std::list of '{\tt <score-part>}'s with their '{\tt id}' attribute, here '{\tt P1}' alone.
...
A '{\tt <part-group>}' element has a '{\tt type}' attribute, whose value can be '{\tt start}' or '{\tt stop}'. A~part group is thus delimited by a std::pair of '{\tt <part-group>}' elements, the first one of type '{\tt start}', and the second one of type '{\tt stop}'.
...
There are elements that are not displayed by the usual music score editors, because there is no "standard" way to do so. One of them is the scordatura used on std::string instrument.

\sep

For example, the scordatura in \gitmxmlfile{strings/Scordatura.xml} is the case where the sixth std::string of the guitar is tuned a tone down to D, which can be described by:\\
