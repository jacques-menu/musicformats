...
\MakeUppercase{All segments} in \mf\ \MakeUppercase{is done internally}: the \class{msrSegment} instances are created in voices and repeats \MakeUppercase{behind the curtains}.
...
\Method{msrVoice}{createMeasureRepeatFromItsFirstMeasures} is presented in \sectionRef{Measure repeats}, and the remaining two are presented in the next sections.
...
\item \method{msrVoice}{handleMultiMeasureRestsStartInVoiceClone}\\[-0.5ex]
...
All the uses of this method concern repeats (\sectionRef{Repeats}), measure repeats (\sectionRef{Measure repeats}) and multi-measure rests(\sectionRef{Multi-measure rests}).
