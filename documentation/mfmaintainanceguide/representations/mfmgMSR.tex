...
\begin{lstlisting}[language=CPlusPlus]
    mfRational              fCurrentMeasureCurrentPositionInMeasure;
...
\EnumType{mfDurationKind} is declared in \msr{mfDurations.h}:
\begin{lstlisting}[language=CPlusPlus]
...
\Class{mfMoment} stores a position in a measure, with a relative offset since harmonies can be placed on a note during its sounding time:
\begin{lstlisting}[language=CPlusPlus]
...
They are represented in MSR with the \enumType{mfDurationKind} enumeration type, defined in \msr{msrBasicTypes.h}:
\begin{lstlisting}[language=CPlusPlus]
// durations
//______________________________________________________________________________
enum class mfDurationKind {
...
\Class{mfDottedNotesDuration} contains:
\begin{lstlisting}[language=CPlusPlus]
...
class EXP msrTempoTuplet : public msrElement
{
  public:
    ...
    static SMARTP<msrTempoTuplet> create (
      ...
      mfWholeNotes            memberNotesDisplayWholeNotes);
...
class EXP msrTempo : public msrMeasureElement
{
  ...
  private:
    ...
    mfDottedNotesDuration     fTempoBeatUnit;

    std::string           fTempoPerMinute; // '90' or '132-156' for example
    mfDottedNotesDuration     fTempoEquivalentBeatUnit;
...
Multiple measure rests handling is presented at \sectionRef{Multiple measure rests handling}.

\Class{msrMultipleMeasureRest}, defined in \msrBoth{msrMultipleMeasureRest}, essentially contains a liste of \class{_msrMeasure} instances and a multiple measure rests number:
\begin{lstlisting}[language=CPlusPlus]
