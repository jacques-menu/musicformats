...
#   echo "FILENAME_WITHOUT_SUFFIX: ${FILENAME_WITHOUT_SUFFIX}"
  ...
  MUSICXML_FILE_NAME="${FILENAME_WITHOUT_SUFFIX}.xml"
  if [ -f ${MUSICXML_FILE_NAME} ]; then
    echo "--> moving ${MUSICXML_FILE_NAME} to ${VALIDATED_DIR_NAME}"
    mv "${MUSICXML_FILE_NAME}" "${VALIDATED_DIR_NAME}"
  else
    echo "MUSICXML_FILE_NAME ${MUSICXML_FILE_NAME} does not exist"
  fi

  MUSICXML_FILE_NAME="${FILENAME_WITHOUT_SUFFIX}.musicxml"
  if [ -f ${MUSICXML_FILE_NAME} ]; then
    echo "--> moving ${MUSICXML_FILE_NAME} to ${VALIDATED_DIR_NAME}"
    mv "${MUSICXML_FILE_NAME}" "${VALIDATED_DIR_NAME}"
  else
  ...
  if [ -f ${LILYPOND_FILE_NAME} ]; then
    echo "--> moving ${LILYPOND_FILE_NAME} to ${VALIDATED_DIR_NAME}"
    mv "${LILYPOND_FILE_NAME}" "${VALIDATED_DIR_NAME}"
  else
  ...
  if [ -f ${PDF_FILE_NAME} ]; then
    echo "--> moving ${PDF_FILE_NAME} to ${VALIDATED_DIR_NAME}"
    mv "${PDF_FILE_NAME}" "${VALIDATED_DIR_NAME}"
  else
    echo "PDF_FILE_NAME ${PDF_FILE_NAME} does not exist"
  fi

  MIDI_FILE_NAME="${FILENAME_WITHOUT_SUFFIX}.midi"
  if [ -f ${MIDI_FILE_NAME} ]; then
    echo "--> moving ${MIDI_FILE_NAME} to ${VALIDATED_DIR_NAME}"
    mv "${MIDI_FILE_NAME}" "${VALIDATED_DIR_NAME}"
  else
